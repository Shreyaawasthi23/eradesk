...
// Splits a To/Cc header ("Name <a@x.com>, b@y.com") into a list of bare
// email addresses.
...
// and excluding Gmail's Promotions/Social/Updates/Forums tab categories, so
// newsletters and notification mail don't get turned into incidents. Also
// excludes mailer-daemon/postmaster bounce notifications at the query level
// (a message we sent — e.g. an auto-reply — bouncing back to us shouldn't
// become a new incident), with a second check in code as a fallback for
// sender formats Gmail's search doesn't catch.
export async function listUnreadIncomingMessages() {
  const gmail = getGmailClient()

  const query =
    'is:unread in:inbox -from:me category:primary -from:mailer-daemon -from:postmaster'
  ...
  for (const id of ids) {
    ...
    const shaped = toMessageShape(full.data)
    if (isAutomatedBounce(shaped.senderEmail)) continue
