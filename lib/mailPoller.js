...
//        YES -> check that serial's linked Purchase Order. If expired
//               (status:false / past endDate), no incident is created —
//               just send the "expired" reply. Otherwise create a new
//               incident and send the normal "new ticket" reply. Either
//               way the Sales Team is cc'd.
...
export async function pollSupportMailbox(db) {
  ...
  for (const message of messages) {
    try {
      ...
      // Serial recognized — check whether its linked Purchase Order has
      // expired before deciding whether to create an incident.
      ...
      if (poExpired) {
        // AMC/support contract has expired — notify the customer, but do
        // not create an incident.
        if (settings.autoReplyEnabled) {
          const template = settings.expiredTicketTemplate || DEFAULT_EXPIRED_TICKET_TEMPLATE
          const body = fillTemplate(template, { name: message.senderName, serialNumber: matchedSerial })
          const ccList = buildCcList(message, supportEmail, salesEmails)
        ...
        await markMessageRead(message.messageId)
        results.push({ from: message.senderEmail, serialNumber: matchedSerial, action: 'expired_no_incident' })
        continue
      }
      ...
      if (settings.autoReplyEnabled) {
        const template = settings.newTicketTemplate || DEFAULT_NEW_TICKET_TEMPLATE
      ...
      await markMessageRead(message.messageId)
      created++
      results.push({ incidentId, from: message.senderEmail, action: 'created' })
    } catch (error) {
