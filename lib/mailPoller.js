...
import { isGraphMailConfigured, listUnreadIncomingMessages, markMessageRead, sendReply } from '@/lib/graphMail'
...
// if both a FrontClient and EndClient match, their Sales Teams are
// combined. Otherwise the incident is still created, just with no Sales
// Team CC.
//
// Replies within a thread that already produced an incident (e.g. someone
// replying to our auto-reply, or replying-all on the original thread) are
// skipped entirely — no second incident, no second auto-reply.
export async function pollSupportMailbox(db) {
  ...
  const supportEmail = (process.env.GRAPH_MAILBOX || '').toLowerCase()
  ...
  for (const message of messages) {
    try {
      const already = await db.collection('Incident').findOne({ sourceGraphId: message.graphId })
      if (already) {
        await markMessageRead(message.graphId)
        continue
      }
      ...
      if (settings.autoReplyEnabled) {
        ...
        // Everyone the client originally addressed (To + CC, minus the
        // support mailbox itself and the sender, who's already in "To")
        // stays on the thread, alongside the matched client's Sales Team.
        const originalRecipients = [...message.toEmails, ...message.ccEmails]
          .map((e) => e.toLowerCase())
          .filter((e) => e && e !== supportEmail && e !== message.senderEmail.toLowerCase())
        const ccList = Array.from(new Set([...originalRecipients, ...salesEmails]))
