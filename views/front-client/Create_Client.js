...
import { createFrontClient } from '@/apiClients/frontclient_api'
...
const Create_Client = () => {
  ...
  const validateUserName = async (value) => {
    ...
    try {
      ...
      const response = await fetch(
        apiUrl + '/auth/core/front-client/check-by-name?name=' + filteredValue,
        requestOptions,
      )
  ...
  const getFrontCLients = async (page, size, filters = { search, status: statusFilter }) => {
    ...
    const url =
      apiUrl +
      '/auth/core/front-client/get-all?page=' +
      page +
      '&size=' +
      size +
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      myHeaders.append('X-Tenant', '' + tenant + '')
      myHeaders.append('Authorization', 'Bearer ' + details?.token + '')
      const response = await fetch(apiUrl + '/auth/core/front-client/get-all-list?status=all', {
        method: 'GET',
        headers: myHeaders,
        redirect: 'follow',
      })
